...
{
    {.src_img = &IMG_Car,        .text = "Remote",  .pageID = PAGE_Handshake},
		{.src_img = &IMG_Game1,      .text = "Game",    .pageID = PAGE_Game},
...
//    {.src_img = &IMG_Stopwatch2, .text = "Stopwatch",.pageID = PAGE_StopwatchCfg},
...
static void ICON_Grp_Creat(lv_obj_t * par)
{
    ...
    style_cont.body.border.color = LV_COLOR_SILVER;
    style_cont.body.radius = 7;
...
static void Task_chUpdate(lv_task_t * task)
{
//	  int16_t old_R_x;
//	  RCX::ChannelUpdate();
//    int16_t R_x = RCX::ChannelRead(4);
//	  int16_t R_y = RCX::ChannelRead(5);
//		if(R_x>900 && abs(old_R_x-R_x)>100)
//		{
//				ICON_Grp_Move(+1);
//			  old_R_x=R_x;
//		}
...
static void Setup()
{
    ...
    ICON_Grp_MoveFouce(ICON_NowSelIndex);
	  taskch = lv_task_create(Task_chUpdate, 10, LV_TASK_PRIO_LOW, 0);
}
